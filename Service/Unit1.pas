...
uses
  ...
  IBSQL, IdBaseComponent, IdComponent, IdTCPConnection, IdTCPClient, IdFTP,
  ExtCtrls, IBDatabase, DB, Math, Variants, StdCtrls, StrUtils;

type
  TRead_azovstal_ftp = class(TService)
...
var
  Read_azovstal_ftp: TRead_azovstal_ftp;
  ms:  TMemoryStream;
...
begin
  Read_azovstal_ftp.Controller(CtrlCode);
end;

function TRead_azovstal_ftp.GetServiceController: TServiceController;
begin
  Result := ServiceController;
end;

procedure TRead_azovstal_ftp.Timer_FTPTimer(Sender: TObject);
var
 ...
 ms:=TMemoryStream.Create;
  ...
  try
    ...
    try
      ...
      ftp.Host:='62.221.58.170';// ftp адрес сервера
    ...
    ShowMessage('Неудачная попытка скачивания');

  end;

end;

procedure TRead_azovstal_ftp.Timer_BDTimer(Sender: TObject);
var
List: TStringList;
sr: TSearchRec;
...
List.Free;
ShowMessage('Неудачная попытка записи в базу');
...
procedure TRead_azovstal_ftp.ServiceStart(Sender: TService; var Started: Boolean);
begin
